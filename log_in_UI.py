from tkinter import *

root = Tk()
root.title("Lost and Found Desk - Login")
root.geometry("400x300")

# Creating a frame to box the login form
login_frame = Frame(root, relief="raised", bd=2, padx=20, pady=20)
login_frame.grid(row=0, column=0, padx=50, pady=50, sticky="e")

# entry fields for username and password using grid within the frame
Label(login_frame, text="Username:").grid(row=0, column=0, padx=10, pady=10, sticky="e")
username_entry = Entry(login_frame, width=20, relief="sunken", bd=2)
username_entry.grid(row=0, column=1, padx=10, pady=10)

Label(login_frame, text="Password:").grid(row=1, column=0, padx=10, pady=10, sticky="e")
password_entry = Entry(login_frame, show="*", width=20, relief="sunken", bd=2)
password_entry.grid(row=1, column=1, padx=10, pady=10)

# login button
login_button = Button(login_frame, text="Login")
login_button.grid(row=2, column=0, columnspan=2, pady=20)

root.mainloop()